...
describe('FileSystemDatasource', () => {
  ...
  test('should throw an error if severity level is not defined', async () => {
    const LogDataSource = new FileSystemDatasource()
    const customSeveryLevel = 'SUPER_MEGA_HIGH' as LogSeverityLevel

    try {
      await LogDataSource.getLogs(customSeveryLevel)
      expect(true).toBeFalsy
    } catch (error) {
      const errorString = `${error}`
      expect(errorString).toEqual(`Error: ${customSeveryLevel} not implemented`)
    }
  })
})
